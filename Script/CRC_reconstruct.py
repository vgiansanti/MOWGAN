...
rna_read = 'CRC_RNA.h5ad'  #str(sys.argv[1]) # rna_data
...
# a pandas dataframe to save the loss information to
    ...
    model.load_weights(path1+'MOWGAN_CRC_SPECTRAL_BATCH_'+str(i))
    
    cells_max = max(rna_sub.shape[0],atac_sub.shape[0])
    cells_d1 = min(cells_max,rna_sub.shape[0])
    cells_d2 = min(cells_max,atac_sub.shape[0])

    samples = model.generate(tf.random.normal(shape=(cells_max,2, N_Z)))

    data_rna = np.array(samples[:cells_d1, 0])
    data_atac = np.array(samples[:cells_d2, 1])

    rna_sub.obsm['X_MOWGAN'] = scaler_rna.inverse_transform(data_rna) #+T_rna
    atac_sub.obsm['X_MOWGAN'] = scaler_atac.inverse_transform(data_atac) #+T_atac
    
        
    neigh_rna = KNeighborsRegressor(n_neighbors=2) #int(np.sqrt(rna.shape[0]/2))
    neigh_rna.fit(rna_sub.obsm['X_pca'][:,:20], rna_sub.X)
    
    neigh_atac = KNeighborsRegressor(n_neighbors=2) #int(np.sqrt(rna.shape[0]/2))
    neigh_atac.fit(atac_sub.obsm['X_ttd'][:,:20], atac_sub.X.todense())
    
    rna_sub.layers['X_MOWGAN'] = neigh_rna.predict(rna_sub.obsm['X_MOWGAN'])
    atac_sub.layers['X_MOWGAN'] = neigh_atac.predict(atac_sub.obsm['X_MOWGAN'])
    
    rna_data.append(rna_sub)
    atac_data.append(atac_sub)

r_conc = rna_data[0].concatenate([rna_data[1],rna_data[2]])
a_conc = atac_data[0].concatenate([atac_data[1],atac_data[2]])
...
path1 = path1
r_conc.write(path1+'RNA_MOWGAN.h5ad')
a_conc.write(path1+'GET_MOWGAN.h5ad')
